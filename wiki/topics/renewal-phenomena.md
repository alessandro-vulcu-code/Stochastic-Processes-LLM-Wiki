---
type: topic
tags: [renewal-process, regenerative-processes, semi-markov-processes]
sources: [lecture-notes-ch5, stochastic-exercises-guida-semplice]
---

# Renewal Phenomena

## Overview

Renewal theory studies processes that statistically restart at event times. It generalizes Poisson arrivals by allowing arbitrary i.i.d. interrenewal distributions, and it turns long-run time averages into ratios of one-cycle expectations.

## Core Concepts

- [[concepts/renewal-process|Renewal Process]]
- [[concepts/renewal-function|Renewal Function]]
- [[concepts/excess-life|Excess Life]]
- [[concepts/backward-recurrence-time|Backward Recurrence Time]]
- [[concepts/sampling-paradox|Sampling Paradox]]
- [[concepts/renewal-equation|Renewal Equation]]
- [[concepts/delayed-renewal-process|Delayed Renewal Process]]
- [[concepts/alternating-renewal-process|Alternating Renewal Process]]
- [[concepts/renewal-reward-process|Renewal Reward Process]]
- [[concepts/regenerative-process|Regenerative Process]]
- [[concepts/semi-markov-process|Semi-Markov Process]]

## Key Theorems

- [[theorems/elementary-renewal-theorem|Elementary Renewal Theorem]]
- [[theorems/basic-renewal-theorem|Basic Renewal Theorem]]
- [[theorems/finiteness-renewal-function|Finiteness of the Renewal Function]]
- [[theorems/solution-renewal-equation|Solution of the Renewal Equation]]
- [[theorems/renewal-reward-theorem|Renewal Reward Theorem]]
- [[theorems/semi-markov-long-run-distribution|Semi-Markov Long-Run Distribution]]
- [[theorems/basic-limit-theorem-renewal-proof|Basic Limit Theorem via Renewal Theory]]

## Suggested Reading Order

1. [[sources/lecture-notes-ch5]]
2. [[concepts/renewal-process]]
3. [[concepts/renewal-function]]
4. [[concepts/sampling-paradox]]
5. [[concepts/renewal-equation]]
6. [[theorems/finiteness-renewal-function]]
7. [[theorems/solution-renewal-equation]]
8. [[theorems/elementary-renewal-theorem]]
9. [[concepts/alternating-renewal-process]]
10. [[theorems/renewal-reward-theorem]]
11. [[concepts/semi-markov-process]]
12. [[exercises/failure-wald-like-identity]]
13. [[exercises/excess-life-triangular-lifetime]]
14. [[sources/stochastic-exercises-guida-semplice]]
15. [[exercises/stochastic-exercises-guida-semplice]]

## Open Problems / Active Research

> [!todo] Gap - Add a dedicated comparison page for ordinary, delayed, stationary, alternating, and regenerative renewal structures.
