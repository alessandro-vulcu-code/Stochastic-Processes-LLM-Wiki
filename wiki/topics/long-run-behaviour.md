---
type: topic
tags: [markov-chains, recurrence, ergodic-theory]
sources: [lecture-notes-ch3, lecture-notes-ch5, stochastic-exercises-guida-semplice]
---

# Long Run Behaviour of Markov Chains

## Overview

This topic classifies Markov-chain states and classes according to accessibility, communication, periodicity, recurrence, and return times. The main objective is to determine when $P_{ij}^{(n)}$ converges, what it converges to, and how reducible or periodic structure changes the answer.

## Core Concepts

- [[concepts/communicating-classes|Communicating Classes]]
- [[concepts/irreducible|Irreducible]]
- [[concepts/period|Period]]
- [[concepts/aperiodic|Aperiodic]]
- [[concepts/recurrence|Recurrence]]
- [[concepts/positive-recurrent|Positive Recurrent]]
- [[concepts/null-recurrent|Null Recurrent]]
- [[concepts/state-classification|State Classification]]
- [[concepts/stationary-distribution|Stationary Distribution]]
- [[concepts/limiting-probability-distribution|Limiting Probability Distribution]]
- [[concepts/random-walk|Random Walk]]
- [[concepts/renewal-process|Renewal Process]]

## Key Theorems

- [[theorems/limiting-distribution-regular-markov-chain|Limiting Distribution of a Regular Markov Chain]]
- [[theorems/period-class-property|Period Is a Class Property]]
- [[theorems/basic-limit-theorem|Basic Limit Theorem]]
- [[theorems/recurrence-criterion|Recurrence Criterion]]
- [[theorems/stationary-distribution-positive-recurrent-aperiodic|Stationary Distribution for a Positive Recurrent Aperiodic Class]]
- [[theorems/transient-to-recurrent-limit|Transient-to-Recurrent Limit Theorem]]
- [[theorems/transience-criterion|Transience Criterion]]
- [[theorems/basic-limit-theorem-renewal-proof|Basic Limit Theorem via Renewal Theory]]

## Suggested Reading Order

1. [[sources/lecture-notes-ch3]]
2. [[concepts/communicating-classes]]
3. [[concepts/irreducible]]
4. [[concepts/period]]
5. [[concepts/aperiodic]]
6. [[concepts/recurrence]]
7. [[concepts/positive-recurrent]]
8. [[concepts/null-recurrent]]
9. [[concepts/limiting-probability-distribution]]
10. [[theorems/basic-limit-theorem]]
11. [[theorems/stationary-distribution-positive-recurrent-aperiodic]]
12. [[sources/lecture-notes-ch5]] for the renewal-theoretic proof.
13. [[exercises/g-m-1-positive-recurrence]]
14. [[exercises/m-g-1-classification]]
15. [[sources/stochastic-exercises-guida-semplice]]
16. [[exercises/stochastic-exercises-guida-semplice]]

## Open Problems / Active Research

> [!todo] Gap - Add a visual decision tree for state classification across random walks and queues.
