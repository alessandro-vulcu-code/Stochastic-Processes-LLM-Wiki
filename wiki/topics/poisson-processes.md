---
type: topic
tags: [poisson-process, continuous-time, queueing]
sources: [lecture-notes-ch4, stochastic-exercises-guida-semplice]
---

# Poisson Processes

## Overview

Poisson processes are continuous-time counting processes with independent stationary increments and Poisson-distributed counts. They are the canonical model for memoryless arrivals and rare-event aggregation.

## Core Concepts

- [[concepts/poisson-process|Poisson Process]]
- [[concepts/non-homogeneous-poisson-process|Non-Homogeneous Poisson Process]]
- [[concepts/poisson-distribution|Poisson Distribution]]
- [[concepts/exponential-distribution|Exponential Distribution]]
- [[concepts/gamma-distribution|Gamma Distribution]]
- [[concepts/m-g-infinity-queue|M/G/infinity Queue]]
- [[concepts/backward-recurrence-time|Backward Recurrence Time]]
- [[concepts/pasta|PASTA]]

## Key Theorems

- [[theorems/poisson-sum|Sum of Independent Poisson Variables]]
- [[theorems/poisson-binomial-filter|Binomial Filter of a Poisson Variable]]
- [[theorems/poisson-approximation-bound|Poisson Approximation Bound]]
- [[theorems/superposition-theorem|Superposition Theorem]]
- [[theorems/thinning-theorem|Thinning Theorem]]
- [[theorems/poisson-interarrival-exponential|Poisson Inter-Arrivals Are Exponential]]
- [[theorems/waiting-times-gamma|Waiting Times Are Gamma]]
- [[theorems/conditional-arrival-times-uniform|Conditional Arrival Times Are Ordered Uniforms]]
- [[theorems/binomial-conditional-distribution|Binomial Conditional Distribution]]
- [[theorems/pasta|PASTA]]

## Suggested Reading Order

1. [[sources/lecture-notes-ch4]]
2. [[concepts/poisson-process]]
3. [[theorems/superposition-theorem]]
4. [[theorems/thinning-theorem]]
5. [[theorems/conditional-arrival-times-uniform]]
6. [[concepts/m-g-infinity-queue]]
7. [[concepts/pasta]]
8. [[exercises/two-independent-poisson-processes]]
9. [[exercises/dispatching-system]]
10. [[sources/stochastic-exercises-guida-semplice]]
11. [[exercises/stochastic-exercises-guida-semplice]]

## Open Problems / Active Research

> [!todo] Gap - Audit OCR-damaged Chapter 4 arithmetic and figure-dependent arguments.
